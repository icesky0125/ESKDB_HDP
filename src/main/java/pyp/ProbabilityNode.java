...
public final class ProbabilityNode {
	/**
	 * True count
	 */
	int[] nk;
		...
		if (parent == null) {
			...
			for (int k = 0; k < nk.length; k++) {
				if (nk[k] <= 1) {
					...
					tk[k] = (int) Math.max(
							1,
							Math.floor(concentration/ d
									* (tree.getRisingFact(concentration + d, nk[k])
											/ tree.getRisingFact(concentration, nk[k]) - 1)));
				} else {// DP case
					tk[k] = (int) Math.max(
							1,
...
//		System.out.println(res+" c="+concentration+" N="+marginal_nk);
		...
		// Now nks are set for current node; let's initialize the tks
		for (int k = 0; k < nk.length; k++) {
			res += tree.logStirling(d, nk[k], tk[k]);
	...
	public void setDiscountRecursively(double d) {
		...
		if (children != null) {
			for (int c = 0; c < children.length; c++) {
				if (children[c] != null) {
					children[c].setDiscountRecursively(d);
				}
			}
		}
	}
	...
	public String printTksAndNksRecursively(String prefix) {
		...
		// root node
		res += prefix+ " :nk=" + Arrays.toString(nk)  + ":tk=" + Arrays.toString(tk) + " :pk=" + Arrays.toString(this.pkAccumulated) + " :c=" + this.c
				+ "\n";
		if (children != null) {
			for (int c = 0; c < children.length; c++) {
				if (children[c] != null) {
	...
	public String printAccumulatedPksRecursively(String prefix) {
		...
		// root node
		res += prefix + ":nk=" + Arrays.toString(nk) + ":tk=" + Arrays.toString(tk)+":pk=" + Arrays.toString(pkAccumulated) + " c=" + this.c + "\n";
		
//		res += prefix +":nk=" + Arrays.toString(nk) +":pk=" + Arrays.toString(pkAccumulated) + "\n";
		if (children != null) {
			for (int c = 0; c < children.length; c++) {
				if (children[c] != null) {
	...
	protected double setTk(int k, int val) {
		...
		if (parent != null) {
			...
			res += tree.logStirling(parent.d, parent.nk[k],  parent.tk[k]);
...
//					}	
	...
	public void convertCountToProbs() {		
		if (isLeaf()) {
			...
			for(int i = 0; i < nk.length; i++){
				pkAccumulated[i] = SUtils.MEsti(nk[i], marginal_nk, nk.length);
			}
		} else {
			// else just call recursively
			if (children != null) {
	...
	public void convertCountToProbsBackOff() {
		...
		if(Utils.sum(nk) != 0){
			...
			if(this.m_BackOff){
				pkAccumulated = this.parent.pkAccumulated;
			}else{
				for(int i = 0; i < nk.length; i++){
					pkAccumulated[i] = (double)1/nk.length;
				}
			}
		}
